...
if [[ "$MODE" != "stereo" && "$MODE" != "mono" ]]; then
    echo "Usage: bash scripts/run_tartanair_8seq_holdout_full_metrics.sh [stereo|mono]"
    exit 2
fi
...
LOG_DIR="$ROOT_DIR/batch_logs/${MODE}_8seq_holdout_full_metrics_color_refinement"
mkdir -p "$LOG_DIR"

STAMP="$(date +%Y%m%d_%H%M%S)"
STATUS_FILE="$LOG_DIR/status_${STAMP}.txt"
SUMMARY_CSV="$LOG_DIR/summary_${MODE}_SE000_SE003_SH000_SH003_${STAMP}.csv"
...
echo "MonoGS ${MODE} 8-sequence full benchmark started: $(date)" | tee "$STATUS_FILE"
echo "Protocol: 80/20 holdout + original 26000-iter color refinement + SE(3) ATE" | tee -a "$STATUS_FILE"
echo "Timing: Online Time excludes final evaluation; Offline Refine is color refinement only; System Total = Online + Offline" | tee -a "$STATUS_FILE"
...
for SEQ in "${SEQUENCES[@]}"; do
    LOG_FILE="$LOG_DIR/${SEQ}_${MODE}_full_metrics_${STAMP}.log"

    echo "============================================================" | tee -a "$STATUS_FILE"
    echo "START: $MODE $SEQ full sequence" | tee -a "$STATUS_FILE"
