...
LOG_DIR="$ROOT_DIR/batch_logs/mono_holdout_80_20"
mkdir -p "$LOG_DIR"

STAMP="$(date +%Y%m%d_%H%M%S)"
STATUS_FILE="$LOG_DIR/status_${STAMP}.txt"
SUMMARY_CSV="$LOG_DIR/summary_MONO_SH000_SH003_full_holdout_80_20_${STAMP}.csv"

echo "Sequence,MaxMap,Train PSNR,Train SSIM,Test PSNR,Test SSIM,ATE(m),FPS,Gaussians" > "$SUMMARY_CSV"

echo "MonoGS monocular batch started: $(date)" | tee "$STATUS_FILE"

auto_find_row() {
    local seq="$1"
    find "$ROOT_DIR/results/tartanair_mono_${seq}" \
        -type f \
...
for SEQ in "${SEQUENCES[@]}"; do
    LOG_FILE="$LOG_DIR/${SEQ}_full_mono_holdout_80_20_${STAMP}.log"

    echo "============================================================" | tee -a "$STATUS_FILE"
    echo "START: MonoGS-Mono $SEQ full sequence, train:test=4:1" | tee -a "$STATUS_FILE"
    echo "TIME : $(date)" | tee -a "$STATUS_FILE"
    echo "LOG  : $LOG_FILE" | tee -a "$STATUS_FILE"
    echo "============================================================" | tee -a "$STATUS_FILE"

    python run_tartanair_mono_holdout_safe_entry.py \
...
echo "============================================================" | tee -a "$STATUS_FILE"
echo "MonoGS monocular batch finished: $(date)" | tee -a "$STATUS_FILE"
